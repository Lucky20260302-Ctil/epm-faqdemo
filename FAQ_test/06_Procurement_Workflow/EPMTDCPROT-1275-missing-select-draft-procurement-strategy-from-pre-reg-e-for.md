---
project: EPMTDCPROT
issue_key: EPMTDCPROT-1275
issue_type: ''
status: ''
title: "EPMTDCPROT-1275: Missing Select Draft Procurement Strategy from Pre-Reg e-form"
tags:
- 06-procurement-workflow
- 06_procurement_workflow
- epm
- epmtdcprot
- faq
symptom: ''
root_cause: ''
solution: ''
jira_url: https://ctil.atlassian.net/browse/EPMTDCPROT-1275
created: ''
resolved: ''
fix_version: ''
components: []
has_images: false
category: 06_Procurement_Workflow
category_label: 採購流程
quality: complete
---

EPMTDCPROT-1275: Missing Select Draft Procurement Strategy from Pre-Reg e-form

## 症狀

為什麼Pre-Reg e-form中缺少Select Draft Procurement Strategy選擇功能？

## 根因

## 解法

## 相關資訊

- Jira: [EPMTDCPROT-1275](https://ctil.atlassian.net/browse/EPMTDCPROT-1275)
- Fix Version: 未標註
- 分類: 採購流程
- 專案: EPMTDCPROT


## 相關截圖

<img src="/FAQ_test/attachments/EPMTDCPROT-1275/image-20250417-053835.jpg" style="max-width:100%;border-radius:6px;margin:4px 0">

<img src="/FAQ_test/attachments/EPMTDCPROT-1275/image-20250603-093633.jpg" style="max-width:100%;border-radius:6px;margin:4px 0">

