---
project: FE
issue_key: FE-1739
issue_type: Bug DEV
status: Closed
faq_score: 5.5
faq_category: 05_Error_Exception
faq_category_label: 錯誤與異常
tags: [faq, fe, error_exception, front-end-v750.01r01a]
jira_url: "https://ctil.atlassian.net/browse/FE-1739"
created: 2025-07-30
resolved: 2025-09-08
resolution: Done
has_images: True
---

# FE-1739: [INC3119292]Posting OSS_B terminated

> **類型:** Bug DEV | **狀態:** Closed
> **分類:** 錯誤與異常 | **FAQ 分數:** 5.5
> **解決日期:** 2025-09-08
> **負責人:** Sang
> **組件:** Front End v750.01R01A

## 問題描述

[INC3119292]Posting OSS_B terminated.

Checked that line34 was seperated in two lines.

> 📎 **image (33).png** — [查看附件](https://ctil.atlassian.net/rest/api/3/attachment/content/554f66f9-d1db-424e-994d-8d18b45c7932)（需 Jira 登入）


## 附件截圖

1. 📎 **image (33).png** — [在 Jira 查看](https://ctil.atlassian.net/rest/api/3/attachment/content/554f66f9-d1db-424e-994d-8d18b45c7932)

## 相關資訊

- **Jira:** [FE-1739](https://ctil.atlassian.net/browse/FE-1739)
- **解決方式:** Done